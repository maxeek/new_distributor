        ...
        <div v-show="hidePreLoader">
            ...
            <div class="row my-5">
                <div
                    ...
                    :key="index"
                    :class="{
                        'col-md-6': filtersData.length === 1 && options.search,
                        'col-12 col-md-4 col-lg-4': (filtersData.length % 3 === 0 && !options.search) || filtersData.length === 5 || (filtersData.length % 2 === 0 && options.search),
                        'col-12 col-md-6 col-lg-3': (filtersData.length % 2 === 0 && !options.search) || (filtersData.length % 3 === 0 && options.search),
                        'col-12 col-md-6 col-lg-6': filtersData.length === 2 && !options.search,
                    }"
                >
                    ...
                    <div class="input-field" v-show="filter.type === 'dropdown'">
                        <select :id="'filter_' + index" class="form-select">
                ...
                <div
                    class="form-group"
                    :class="{
                        'col-md-6 col-lg-6': filtersData.length === 1 && options.search,
                        'col-12 col-md-4 col-lg-4': (filtersData.length > 0 && filtersData.length % 2 === 0) || filtersData.length > 4,
                        'col-12 col-md-6 col-lg-3': filtersData.length > 0 && filtersData.length % 3 === 0,
                        'col-12 col-md-4 col-lg-4 offset-md-8 offset-lg-8': filtersData.length == 0,
                    }"
                    v-if="options.search"
                >
                                ...
                                <thead>
                                    <tr class="fw-bold text-muted">
                                        ...
                                        <th
                                            ...
                                            :class="{
                                                'text-end': column.type === 'component',
                                                'text-end': includes(options.right_align, column.key)
                                            }"
                                        >
                                            ...
                                            >
                                                <div class="data-table-sort-parent">
                                                    <div class="text-start data-table-sort-child">{{ column.title }}</div>

                                                    <div class="text-end data-table-sort-child">
                                ...
                                <tbody v-if="datasets.length > 0">
                                    <tr v-for="(data, index) in datasets" :key="index">
                                        ...
                                        <td
                                            ...
                                            :key="index"
                                            :class="
                                                (index + 2 === datasets.length || index + 1 === datasets.length) && options.summary
                                                ? {
                                                    'font-weight-bold': options.summary,
                                                    'border-bottom-0': index + 1 === datasets.length,
                                                    'text-end': includes(options.right_align, column.key)
                                                }
                                                : {
                                                    'text-end': includes(options.right_align, column.key) || column.type === 'component',
                                                    'table-col-truncate': column.type === 'text' && column.key === 'description'
                                                }
                                            "
                                        >
                                            ...
                                            <span v-else>
                                                ...
                                                <a
                                                    :href="publicPath + '/' + column.source + '/' + data[column.uniquefield] + '?tab_name=' + tab_name + '&&route_name=' + route_name"
                                                    v-else-if="column.type === 'clickable_link' && column.uniquefield && data[column.uniquefield] && !column.dynamicSource"
                                                >{{ data[column.key] }}</a>

                                                <a
                                                    :href="publicPath + '/' + data[column.dynamicSource] + '/' + data[column.uniquefield] + '?tab_name=' + tab_name + '&&route_name=' + route_name"
                                                    v-else-if="column.type === 'clickable_link' && column.uniquefield && data[column.uniquefield] && column.dynamicSource"
                                                >{{ data[column.key] }}</a>

                                                <!-- When the column type is text -->
                                                <span
                                                    v-else-if="column.type === 'text' || column.key === 'description'"
                                                    :class="{
                                                        'table-col-truncate-item': column.type === 'text' && column.key === 'description',
                                                    }"
                                                >{{ data[column.key] }}</span>

                                                <!--When the column type is images-->
                                                <span v-if="(index + 2 === datasets.length || index + 1 === datasets.length) && options.summary"></span>
                                                ...
                                                <span v-if="column.type === 'array'">
                                                    <span v-for="(value, index) in data[column.key]" :key="index">
                                                        {{ value }}<br/>
                                                    </span>
                                                </span>
    ...
    export default {
        ...
        methods: {
                        ...
                        if (instance.paginationLimitIncreased) {
                            ...
                            newData.forEach(function (element) {
                                ...
                                if (
                                    ...
                                    element["cash_receives"] = instance.numberFormat(
                                        element.cash_receives
                                    );
                                    element["cash_sales"] = instance.numberFormat(
                                        element.cash_sales
                                    );
                                    }
                                ...
                                instance.options.dateFormatting.forEach(function (key) {
                                    ...
                                    if (element[key] !== undefined)
                                    element[key] = instance.dateFormats(element[key]);
                                });
                                }
            ...
            * */
            ...
            const checkedData = this.datasets.filter(
